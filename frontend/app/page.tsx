...
export default function Home() {
  ...
  const streamingStatus = React.useMemo(() => {
    ...
    // CRITICAL: Check if this session is in the streaming set
    // If it is, return its streaming status (streaming/submitted)
    // This ensures the stop button shows even when switching between sessions
    if (streamingSessionIds.has(activeSessionId)) {
      return activeSession.streamingStatus || 'streaming'
    }
    
    return 'ready'
  }, [sessions, activeSessionId, streamingSessionIds])
  ...
  useEffect(() => {
    const loadModels = async () => {
      try {
        ...
        console.error('❌ Failed to load models from settings:', error)
      ...
      if (data.success) {
        ...
        const enrichedCollaborations = data.collaborations.map((collab: any) => {
          ...
          return {
            ...
            projectId: collab.collaboration?.projectId || collab.project?.id,
            projectName: collab.project?.name || 'Unknown Project',
            ownerName: project?.ownerName || collab.collaboration?.ownerName || 'Unknown Owner',
            shareToken: collab.collaboration?.shareToken
          }
        })
  ...
  */ // END OLD LOCALSTORAGE CODE
  ...
  // Initialize Supabase auth and load user session
  useEffect(() => {
    const initAuth = async () => {
      try {
        ...
        console.log('🔐 Auth initialized:', session?.user ? `User: ${session.user.email}` : 'Not signed in')
          ...
          onSignOut={async () => {
            try {
              ...
              if (error) {
                ...
                setActiveSessionId(null)
                setUser(null)
                addToast({ id: Date.now().toString(), type: 'success', message: 'Signed out successfully', duration: 3000 })
                ...
                {activeProject ? (
                  activeProject.isMock ? (
                    ...
                    <AutoRefreshTerminal
                      ...
                      terminalUrl={`${typeof window !== 'undefined' ? (process.env.NEXT_PUBLIC_BACKEND_URL || `${window.location.protocol}//${window.location.hostname}:3002`) : 'http://backend:3002'}/api/proxy/${activeProject.id}/terminal-proxy.html`}
