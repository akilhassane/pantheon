...
class ProjectManager {
  ...
  async restoreHttpServersOnStartup() {
    try {
      ...
      const backendContainerName = process.env.BACKEND_CONTAINER_NAME || 'ai-backend';
      ...
      for (const project of projects) {
        ...
        const thirdOctet = projectHash % 256;
        const backendIp = `172.30.${thirdOctet}.253`;  // Backend uses .253 to avoid conflict with shared folder (.1)
        ...
        try {
          ...
          await network.connect({ 
            ...
            EndpointConfig: {
              IPAMConfig: {
                IPv4Address: backendIp
              }
            }
          });
          console.log(`[ProjectManager] 🔗 Connected backend to ${networkName} at ${backendIp} for MCP access`);
        } catch (connectErr) {
        ...
        try {
          const network = this.docker.getNetwork(networkName);
          const toolsApiIp = `172.30.${thirdOctet}.3`;  // Tools API uses .3 to avoid conflict with shared folder (.1) and container (.2)
          await network.connect({ 
            Container: 'windows-tools-api',
            EndpointConfig: {
              IPAMConfig: {
                IPv4Address: toolsApiIp
              }
            }
          });
          console.log(`[ProjectManager] 🔗 Connected windows-tools-api to ${networkName} at ${toolsApiIp}`);
        } catch (connectErr) {
          if (!connectErr.message.includes('already exists')) {
  ...
  async createSharedFolderContainer(projectId, projectName, sharedFolderPath) {
    try {
      ...
      // Each project gets a unique subnet to avoid Docker pool overlap
      // Shared folder will be at X.1, but Windows VM will access it via 172.30.0.1 (NAT)
      const projectHash = parseInt(projectId.substring(0, 8), 16);
      const thirdOctet = projectHash % 256; // Range: 0-255
      const projectSubnet = `172.30.${thirdOctet}.0/24`;
      const projectGateway = `172.30.${thirdOctet}.254`;
      const sharedFolderIp = `172.30.${thirdOctet}.1`;
      ...
      const sourceNginxConfig = path.resolve(__dirname, '..', 'windows-vm-files', 'nginx-shared-folder.conf');
      ...
      const startupScriptSource = path.resolve(__dirname, '..', 'docker', 'shared-folder-startup.sh');
      ...
      const containerConfig = {
        ...
        Cmd: ['/bin/sh', '/startup.sh'], // Run startup script instead of nginx directly
        HostConfig: {
          ...
          Binds: [
            `${hostSharedFolderPath}:/usr/share/nginx/html`,  // Changed to read-write so script can update .env
            `${hostNginxConfigPath}:/etc/nginx/conf.d/default.conf:ro`,
            `${hostStartupScriptPath}:/startup.sh:ro`  // Mount startup script
          ],
              ...
              for (const [containerId, containerInfo] of Object.entries(containers)) {
                if (containerInfo.IPv4Address && containerInfo.IPv4Address.startsWith(sharedFolderIp)) {
                  ...
                  if (containerInfo.Name === 'windows-tools-api') {
                    ...
                    // Reconnect with proper IP (.3)
                    const toolsApiIp = `172.30.${thirdOctet}.3`;
                    await network.connect({ 
                      Container: 'windows-tools-api',
                      EndpointConfig: {
                        IPAMConfig: {
                    ...
                    console.log(`[ProjectManager] ✅ Reconnected windows-tools-api at ${toolsApiIp}`);
                  } else {
  ...
  async setupSharedFolderProxy(container, projectId, sharedFolderIp = null) {
    try {
      ...
      if (!sharedFolderIp) {
        const projectHash = parseInt(projectId.substring(0, 8), 16);
        const thirdOctet = projectHash % 256;
        sharedFolderIp = `172.30.${thirdOctet}.1`;
      }
  ...
  async createProject(userId, projectName, description = null, operatingSystem = 'kali-linux') {
    ...
    try {
      ...
      if (isWindows) {
        ...
        // Each project gets a unique subnet to avoid Docker pool overlap
        // Shared folder will be at X.1, but Windows VM will access it via 172.30.0.1 (DNAT)
        const projectHash = parseInt(projectId.substring(0, 8), 16);
        const thirdOctet = projectHash % 256;
        const projectSubnet = `172.30.${thirdOctet}.0/24`;
        const projectGateway = `172.30.${thirdOctet}.254`;
        ...
        try {
          ...
          console.log(`[ProjectManager] 🔗 Connected backend (${backendContainerName}) to project network ${networkName} at ${backendIp}`);
        } catch (connectErr) {
          // If already connected, that's fine
          if (!connectErr.message.includes('already exists')) {
    ...
    try {
      ...
      if (operatingSystem === 'windows-11' || operatingSystem === 'windows-10') {
        ...
        try {
          ...
          try {
            await network.disconnect({ Container: 'pantheon-windows-tools', Force: true });
  ...
  async initializeProject(projectId, operatingSystem = 'kali-linux') {
    ...
    try {
      ...
      if (isWindows) {
        ...
        if (isWindows) {
          try {
            ...
            console.log(`[ProjectManager] 🔗 Connected windows-tools-api to project network ${networkName} at ${toolsApiIp}`);
          } catch (connectErr) {
            if (!connectErr.message.includes('already exists')) {
              console.warn(`[ProjectManager] ⚠️  Failed to connect windows-tools-api:`, connectErr.message);
            } else {
      ...
      if (isWindows) {
        ...
        // Get tools API URL (will be updated later with container IP)
        toolsApiUrl = process.env.TOOLS_API_URL || 'http://windows-tools-api:3003';
...
`;
      ...
      if (containerIp && isWindows) {
        // Update toolsApiUrl to use container's IP (where socat proxy runs)
        // Windows VM will access Tools API via socat proxy at container's IP
        toolsApiUrl = `http://${containerIp}:8090`;
        console.log(`[ProjectManager] 🔄 Updated Tools API URL to use container IP: ${toolsApiUrl}`);
...
`;
      ...
      if (checkError || !existingProject) {
        ...
        if (isWindows) {
          ...
          try {
            ...
            try {
              await network.disconnect({ Container: 'ai-backend', Force: true });
            } catch (e) { /* already disconnected */ }
  ...
  async buildContainerConfig(projectId, ports) {
    ...
    const windowsContainerIp = isWindows ? (() => {
      const projectHash = parseInt(projectId.substring(0, 8), 16);
      const thirdOctet = projectHash % 256;
      const ip = `172.30.${thirdOctet}.2`; // Windows container uses .2, shared folder uses .1
      console.log(`[ProjectManager] 🔒 Windows container will use IP: ${ip} (shared folder uses .1)`);
      return ip;
    })() : null;
