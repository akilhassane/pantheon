          ...
          if (result.success) {
            ...
            switch (tool) {
              ...
              case 'find_text_on_screen':
                ...
                // Pass through ALL fields from the agent response
                Object.keys(result).forEach(key => {
                  if (key !== 'success') {
                    transformedResult[key] = result[key];
                  }
                ...
                // Create a user-friendly output message
                if (result.found !== undefined) {
                  if (result.found && result.matches && result.matches.length > 0) {
                    // Format matches for display
                    const matchList = result.matches.map((m, i) => 
                      `  ${i + 1}. "${m.text}" at (${m.position.x}, ${m.position.y}) - confidence: ${m.confidence}`
                    ).join('\n');
                    transformedResult.output = `Found ${result.match_count} match(es) for "${result.text}":\n${matchList}`;
                  } else {
                    transformedResult.output = `Text "${result.text}" not found on screen`;
                  }
                } else {
